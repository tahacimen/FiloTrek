...
async function main() {
  ...
  async function createShipmentWithHistory(opts: {
    ...
    // Every shipment that reached HEADING_TO_PICKUP or beyond must have had
    // its price approved to get there for real (see the guard in
    // shipment-status.ts) — keep demo data consistent with that invariant.
  ...
  // --- Active (in-progress) shipments, consistent with the statuses above --
  ...
  // Priced and assigned, but the customer hasn't approved yet — demonstrates
  // the "Fiyatı Onayla" button in the demo data (see PriceApprovalCard).
  await createShipmentWithHistory({
    customerCompanyId: karadenizGida.id,
    supplierCompanyId: anadolu.id,
    vehicleId: an105.id,
  ...
  // A shipment the dispatcher has created but not yet matched to a vehicle/driver
  // (exercises the assignment screen's PENDING -> ASSIGNED transition). Also
  // carries Kapı Rezervasyonu links to demonstrate they're visible to both
  // roles from PENDING onward, well before any vehicle is assigned.
  ...
  console.log("");
  console.log("Giriş bilgileri (tüm kullanıcılar için şifre: %s)", DEMO_PASSWORD);
  console.log("  Tedarikçi 1 (Anadolu Nakliyat): mehmet@anadolunakliyat.com");
  console.log("  Tedarikçi 2 (Marmara Lojistik): ayse@marmaralojistik.com");
  console.log("  Müşteri 1 (Ege Tekstil):      fatma@egetekstil.com");
  console.log("  Müşteri 2 (Karadeniz Gıda):   ali@karadenizgida.com");
  console.log(
    "  Şoför 1 (Anadolu / Hüseyin Arslan): huseyin@anadolunakliyat.com"
  );
  console.log(
    "  Şoför 2 (Marmara / Emre Koç):       emre@marmaralojistik.com"
  );
  console.log(
    "  Nizamiye (Ege Tekstil):            nizamiye@egetekstil.com"
  );
}
